...
import { Save, Plus, Trash2, Scale, Calculator, CheckCircle2, RefreshCw } from 'lucide-react';
...
const ProductionForm: React.FC = () => {
  ...
  const handleSave = async () => {
    ...
    if (hasZero) {
      alert("⚠️ Por favor, preencha todos os pesos dos baldes.");
      return;
    }
          ...
          {entries.map((entry, eIdx) => (
            <div key={eIdx} className="p-6 bg-fuchsia-50/30 rounded-[32px] border border-fuchsia-100/50 space-y-5">
              <div className="space-y-2">
                <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-2">Escolher Sabor</label>
                <select 
                  className="w-full bg-white border-none rounded-2xl p-4 text-sm font-black text-gray-700 shadow-sm outline-none focus:ring-2 focus:ring-fuchsia-200 transition-all appearance-none"
              ...
              <div className="space-y-3">
                <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-2">Pesagem de Baldes (g)</label>
                <div className="grid grid-cols-1 gap-2">
                  {entry.weights.map((w, wIdx) => (
                    <div key={wIdx} className="flex gap-2">
                      <div className="relative flex-1 group">
                        ...
                        <span className="absolute right-4 top-1/2 -translate-y-1/2 text-[10px] font-black text-fuchsia-200 uppercase">Grams</span>
                      </div>
                      <button 
                        onClick={() => removeBucket(eIdx, wIdx)}
                        className="p-4 bg-white text-rose-300 hover:text-rose-500 rounded-2xl border border-fuchsia-50 shadow-sm transition-all"
                      >
                        <Trash2 size={20} />
                      </button>
                    </div>
                  ...
                  <button 
                    ...
                    className="w-full bg-white/50 border-2 border-dashed border-fuchsia-200 text-fuchsia-400 rounded-2xl p-4 flex items-center justify-center gap-2 hover:bg-fuchsia-50 transition-all font-black uppercase text-[10px] tracking-widest"
                  >
