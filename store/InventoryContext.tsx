...
export const InventoryProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const addProductionBatch = async (entries: ProductionEntry[], note: string, date: Date) => {
    ...
    entries.forEach(entry => {
      ...
      validWeights.forEach((w, idx) => {
        newBuckets.push({
          id: `${flavor.initials}-${Date.now().toString().slice(-4)}-${idx}`,
  ...
  const markAsSold = async (id: string) => {
    await persist({ buckets: buckets.map(b => b.id === id ? { ...b, status: 'vendido' as const, grams: 0 } : b) });
  };

  const updateBucketWeight = async (id: string, grams: number) => {
    await persist({ buckets: buckets.map(b => b.id === id ? { ...b, grams: Number(grams) } : b) });
  };
  ...
  return (
    <InventoryContext.Provider value={{ 
      ...
      addProductionBatch, distributeBuckets, saveStoreClosing, markAsSold, deleteBucket, updateBucketWeight, 
      updateFlavor, addCategory, updateCategory, deleteCategory, exportToCSV, resetDatabase, getSuggestedDistribution 
    }}>
      {children}
